...
const debug = require('debug')('cbc:basic-auth-middleware.js');

module.exports = function(req, res, next) {
  debug('inside auth middleware');

  var authHeader = req.headers.authorization;
  if(!authHeader) {
    return next(createError(401, 'error: an authorization header is required'));
  }

  var base64str = authHeader.split('Basic ')[1];
  if (!base64str) {
    return next(createError(401, 'error: a username and a password are required'));
  }
